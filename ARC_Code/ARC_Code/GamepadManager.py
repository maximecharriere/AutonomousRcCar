...
async def EventManager(device):
    async for event in device.async_read_loop():
        if event.type == ecodes.EV_KEY:
            ...
            if event.code == ecodes.ABS_RX: #Joy Droit / Gauche- Droite+
                print("Joy Droit / Gauche- Droite+")
            elif  event.code == ecodes.ABS_RY: #Joy Droit / Haut- Bas+
                print("Joy Droit / Haut- Bas+")
            elif  event.code == ecodes.ABS_X:  #Joy Gauche / Gauche- Droite+
                SteeringCtrl.Angle(myLib.Map(event.value, -2^15, 2^15, 0, 100))
            elif  event.code == ecodes.ABS_Y: #Joy Gauche / Haut- Bas+
                print("Joy Gauche / Haut- Bas+")
